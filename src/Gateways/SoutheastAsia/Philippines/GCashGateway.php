...
class GCashGateway implements PaymentGateway
{
    ...
    {
        $transactionId = 'GCASH_' . Str::random(12);
        ...
        return new PaymentResponse(true, $transactionId, [
            ...
            'mobile' => $mobile,
            'returnUrl' => $data['return_url'] ?? url('/payment/callback/gcash'),
            'status' => 'PENDING',
        ]);
    }
    ...
    {
        return new PaymentResponse(true, 'REF_GCASH_' . Str::random(12), [
            'gateway' => 'gcash',
    ...
    {
        $subscriptionId = 'SUB_GCASH_' . Str::random(12);
    ...
    {
        $data = $request->all();
        return isset($data['external_id']) && isset($data['status']);
    }
}
