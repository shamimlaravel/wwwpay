...
class LemonSqueezyGateway implements PaymentGateway
{
    ...
    {
        $transactionId = 'LEMON_' . Str::random(12);
        ...
        return new PaymentResponse(true, $transactionId, [
            ...
            'email' => $data['email'] ?? '',
            'returnUrl' => $data['return_url'] ?? url('/payment/callback/lemonsqueezy'),
            'status' => 'PENDING',
        ]);
    }
    ...
    {
        return new PaymentResponse(true, 'REF_LEMON_' . Str::random(12), [
            'gateway' => 'lemonsqueezy',
    ...
    {
        return new PaymentResponse(true, $transactionId, ['gateway' => 'lemonsqueezy', 'status' => 'CANCELLED']);
    }

    public function verify(array $data): bool { return isset($data['order_id']); }
    public function getName(): string { return 'lemonsqueezy'; }
    public function supportsSubscriptions(): bool { return true; }
    public function subscribe(array $data): \ShamimStack\WwwPay\Contracts\Subscription
    {
        $subscriptionId = 'SUB_LEMON_' . Str::random(12);
        return new \ShamimStack\WwwPay\Subscriptions\Subscription(
            $subscriptionId, 'lemonsqueezy', $data['amount'] ?? 0, 'monthly', 'active'
        );
    }
