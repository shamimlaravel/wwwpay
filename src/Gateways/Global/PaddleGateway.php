...
class PaddleGateway implements PaymentGateway
{
    ...
    {
        $transactionId = 'PADDLE_' . Str::random(12);
        ...
        return new PaymentResponse(true, $transactionId, [
            ...
            'email' => $data['email'] ?? '',
            'returnUrl' => $data['return_url'] ?? url('/payment/callback/paddle'),
            'status' => 'PENDING',
        ]);
    }
    ...
    {
        return new PaymentResponse(true, 'REF_PADDLE_' . Str::random(12), [
            'gateway' => 'paddle',
    ...
    {
        return new PaymentResponse(true, $transactionId, ['gateway' => 'paddle', 'status' => 'CANCELLED']);
    }

    public function verify(array $data): bool { return isset($data['order_id']); }
    public function getName(): string { return 'paddle'; }
    public function supportsSubscriptions(): bool { return true; }
    public function subscribe(array $data): \ShamimStack\WwwPay\Contracts\Subscription
    {
        $subscriptionId = 'SUB_PADDLE_' . Str::random(12);
        return new \ShamimStack\WwwPay\Subscriptions\Subscription(
            $subscriptionId, 'paddle', $data['amount'] ?? 0, 'monthly', 'active'
        );
    }
